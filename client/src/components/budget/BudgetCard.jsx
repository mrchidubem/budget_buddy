    ...
    <article className="bb-card-interactive p-4 sm:p-5 min-w-0">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          <div className="w-10 h-10 rounded-[8px] bg-[#9cff6d] text-[#071b16] text-xs font-semibold flex items-center justify-center shrink-0">
      ...
      <div className="mt-5 rounded-[8px] bg-white/[0.04] p-4 ring-1 ring-white/10">
        <div className="flex items-end justify-between gap-3">
          <div className="min-w-0">
            <p className="bb-metric-label">Spent</p>
            <p className={`mt-1 truncate text-xl font-semibold tabular-nums ${isOver ? 'text-red-300' : 'text-white'}`}>
              {formatAmount(spent)}
            </p>
          </div>
          <div className="text-right">
            <p className="bb-metric-label">Limit</p>
            <p className="mt-1 text-sm font-semibold tabular-nums text-[#9aa8a1]">
              {formatAmount(limit)}
            </p>
          </div>
        </div>
      ...
      <dl className="mt-3 grid grid-cols-2 gap-2">
        <div className="min-w-0 p-3 rounded-[8px] bg-white/[0.04] border border-white/10">
          <dt className="bb-metric-label">Remaining</dt>
          <dd className={`mt-1 text-sm font-semibold tabular-nums truncate ${remaining < 0 ? 'text-red-300' : 'text-[#9cff6d]'}`}>
            {formatAmount(Math.max(0, remaining))}
          </dd>
        </div>
        <div className="min-w-0 p-3 rounded-[8px] bg-white/[0.04] border border-white/10">
          <dt className="bb-metric-label">Threshold</dt>
          <dd className="mt-1 text-sm font-semibold text-white tabular-nums truncate">{threshold}%</dd>
        </div>
      </dl>
